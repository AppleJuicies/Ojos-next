...
export default function Dashboard() {
  ...
  const fetchMeetings = useCallback(async () => {
    ...
    try {
      ...
      setMeetings(all.map(m => ({ ...m, accentColor: STATUS_COLOR[m.status] || '#002fa7' })));
      setLoading(false);

      // Lazy-load accent colors
      const otherIds = [...new Set(all.map(m => m.role === 'host' ? m.requester_id : m.host_id).filter(Boolean))];
      if (!otherIds.length) { saveCache(uid, all.map(m => ({ ...m, accentColor: STATUS_COLOR[m.status] || '#002fa7' }))); return; }

      const { data: profiles } = await supabase.from('users').select('id, "accentColor"').in('id', otherIds);
      const colorMap = {};
      (profiles || []).forEach(p => { colorMap[p.id] = p.accentColor || '#002fa7'; });
      const withColors = all.map(m => ({ ...m, accentColor: colorMap[m.role === 'host' ? m.requester_id : m.host_id] || '#002fa7' }));
      setMeetings(withColors);
      saveCache(uid, withColors);
    } catch (err) {
